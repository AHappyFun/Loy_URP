...
TEXTURE2D_SHADOW(_GAEADirShadowMap);
...
float4x4    _GAEAWorldToShadowMapMatrix;
...
{
    //项目使用BakeShadow和RealShadow混合，但不会覆盖。省略一个变体
//#if defined(LIGHTMAP_SHADOW_MIXING)
    return min(lerp(realtimeShadow, 1, shadowFade), bakedShadow);
//#else
    //return lerp(realtimeShadow, bakedShadow, shadowFade);
    //return min(realtimeShadow, bakedShadow);
    //return min(lerp(realtimeShadow, 1, shadowFade), bakedShadow);
//#endif
}
...
half _GAEAShadowQuality;
half _GAEAShadowFade;
half GaeaLightShadow(float3 positionWS)
{
    float4 shadowCoord =  mul(_GAEAWorldToShadowMapMatrix, float4(positionWS, 1.0));

    ShadowSamplingData shadowSamplingData;
    float width = 1024;
    float height = 1024;
    float invShadowAtlasWidth = 1.0f / width;
    float invShadowAtlasHeight = 1.0f / height;
    shadowSamplingData.softShadowQuality = _GAEAShadowQuality;
    shadowSamplingData.shadowmapSize = float4(invShadowAtlasWidth, invShadowAtlasHeight, width, height);
    shadowSamplingData.shadowOffset0 = half4(-invShadowAtlasWidth, -invShadowAtlasHeight, invShadowAtlasWidth, -invShadowAtlasHeight);
    shadowSamplingData.shadowOffset1 = half4(-invShadowAtlasWidth, invShadowAtlasHeight, invShadowAtlasWidth, invShadowAtlasHeight);

    half4 shadowParams = GetMainLightShadowParams();
    half realtimeShadow =  SampleShadowmapFiltered(TEXTURE2D_SHADOW_ARGS(_GAEADirShadowMap, sampler_LinearClampCompare), shadowCoord, shadowSamplingData);
    //half realtimeShadow = SampleShadowmapFilteredLowQuality(TEXTURE2D_SHADOW_ARGS(_GAEADirShadowMap, sampler_LinearClampCompare), shadowCoord, shadowSamplingData);

    realtimeShadow = lerp(1.0f, realtimeShadow, _GAEAShadowFade);
