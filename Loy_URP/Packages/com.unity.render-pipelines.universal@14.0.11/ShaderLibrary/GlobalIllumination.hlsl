...
TEXTURE2D(_GAEAPlanarReflectMap);
...
#if defined(_GAEA_PLANARREFLECT)
    //float3 reflectedWorldPos = positionWS + reflectVector * 200.0f;
    //float4 clipPos = mul(_ReflectionVP, float4(reflectedWorldPos, 1.0));
    //clipPos.xyz /= clipPos.w;
    //float2 uv = clipPos.xy * 0.5 + 0.5;
    //irradiance = SAMPLE_TEXTURE2D_LOD(_PlanarReflectionTexture, sampler_ScreenTextures_linear_clamp, uv, 0).rgb;
    irradiance = SAMPLE_TEXTURE2D_LOD(_GAEAPlanarReflectMap, sampler_ScreenTextures_linear_clamp, normalizedScreenSpaceUV, 0).rgb;
