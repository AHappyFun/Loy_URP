#ifndef GAEA_FXAA_INCLUDED
#define GAEA_FXAA_INCLUDED
...
{
    return SAMPLE_TEXTURE2D_LOD(_BlitTexture, sampler_LinearClamp, screenUV, 0);
}
...
VaryingsAA DefaultPassVertex(uint vertexID : SV_VertexID)
{
    VaryingsAA output;
...
//FXAA AfterColorGrad
float4 FXAAPassFragment(VaryingsAA input) : SV_TARGET
{
...
//Unity URP的版本移植
float4 FXAAPassFragment2(VaryingsAA input) : SV_TARGET
{
    float4 color  = GetSource(input.screenUV);
    float2 positionNDC = input.screenUV;
    int2   positionSS  = input.screenUV * GetSourceTexelSize().xy;
    color.rgb = ApplyFXAA(color.rgb, positionNDC, positionSS, GetSourceTexelSize(), _BlitTexture, PaperWhite, OneOverPaperWhite);
    return color;
}
