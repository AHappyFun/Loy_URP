Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        #pragma multi_compile_local_fragment _ _GAEA_FXAA
        #pragma multi_compile_local_fragment _ _GAEA_RADIALBLUR
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            int2   positionSS  = uv * GetSourceTexelSize().xy;
            ...
            //GAEA RadialBlur 径向模糊
            #if _GAEA_RADIALBLUR
            {
                color = ApplyGAEARadialBlur(color, uvDistorted, _GAEARadialBlur_Params, _BlitTexture);
            }
            //没开模糊，fxaa生效
            #elif _GAEA_FXAA
                color = ApplyFXAA(color.rgb, positionNDC, positionSS, GetSourceTexelSize(), _BlitTexture, PaperWhite, OneOverPaperWhite);
            #endif
