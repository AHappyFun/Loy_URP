...
half3 ApplyGAEARadialBlur(half3 input, float2 uv, half4 params, TEXTURE2D_X(inputTexture))
{
    half _BlurRadius = params.x;
    half _Iteration = params.y;
    half2 _RadialCenter = params.zw;

    float2 blurVector = (_RadialCenter - uv.xy) * _BlurRadius;
    ...
    {
        acumulateColor += SAMPLE_TEXTURE2D_X(inputTexture, sampler_LinearClamp, uv);
        uv += blurVector;
    }
