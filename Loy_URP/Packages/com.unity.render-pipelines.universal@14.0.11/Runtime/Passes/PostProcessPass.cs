...
namespace UnityEngine.Rendering.Universal
{
    ...
    internal class PostProcessPass : ScriptableRenderPass
    {
        ...
        //GAEA effect settings
        GAEARadialBlur m_GaeaRadialBlur;
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            m_GaeaRadialBlur = stack.GetComponent<GAEARadialBlur>();
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            bool useGAEAFxaa = cameraData.antialiasing == AntialiasingMode.GAEAFXAA;
            ...
            //if (useGAEAFxaa)
            //{
            //    using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.GAEAFxaa)))
            //    {
            //        DoFXAA(ref cameraData, cmd, GetSource(), GetDestination());
            //        Swap(ref renderer);
            //    }
            //}
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                SetupGAEARadialBlur(m_Materials.uber);
                ...
                if(useGAEAFxaa)
                    ConfigGAEAFXAA(ref cameraData, m_Materials.uber);
        ...
        void ConfigGAEAFXAA(ref CameraData cameraData, Material material)
        {
            var camera = cameraData.camera;
            var targetSize =new Vector2Int(cameraData.cameraTargetDescriptor.width, cameraData.cameraTargetDescriptor.height);
            material.SetVector(ShaderConstants._Metrics, new Vector4( targetSize.x, targetSize.y, 1f / targetSize.x, 1f / targetSize.y));
            material.EnableKeyword(ShaderKeywordStrings.GAEA_FXAA);
        }

        void DoFXAA(ref CameraData cameraData, CommandBuffer cmd, RTHandle source, RTHandle destination)
        {
            var camera = cameraData.camera;
            var targetSize =new Vector2Int(cameraData.cameraTargetDescriptor.width, cameraData.cameraTargetDescriptor.height);
            var material = m_Materials.gaeaFXAA;
            switch (cameraData.antialiasingQuality)
            {
                case AntialiasingQuality.Low:
                    material.EnableKeyword(ShaderKeywordStrings.GaeaFXAALow);
                    material.DisableKeyword(ShaderKeywordStrings.GaeaFXAAMedium);
                    break;
                case AntialiasingQuality.Medium:
                    material.DisableKeyword(ShaderKeywordStrings.GaeaFXAALow);
                    material.EnableKeyword(ShaderKeywordStrings.GaeaFXAAMedium);
                    break;
                case AntialiasingQuality.High:
                    material.DisableKeyword(ShaderKeywordStrings.GaeaFXAALow);
                    material.DisableKeyword(ShaderKeywordStrings.GaeaFXAAMedium);
            ...
            material.SetVector(ShaderConstants._Metrics, new Vector4( targetSize.x, targetSize.y, 1f / targetSize.x, 1f / targetSize.y));
            material.SetVector("_FXAAConfig", new Vector4(cameraData.gaeaFxaaFixedThreshold, cameraData.gaeaFxaaRelativeThreshold, cameraData.gaeaFxaaSubpixelBlending, 0));
...
#region GAEA自定义
        ...
        void SetupGAEARadialBlur(Material material)
        {
            var radius = m_GaeaRadialBlur.BlurRadius.value;
            var iteration = m_GaeaRadialBlur.Iteration.value;
            var center = m_GaeaRadialBlur.Center.value;
            var v1 = new Vector4(
                radius, iteration, center.x, center.y
            );
            material.SetVector(ShaderConstants._GAEARadialBlur_Params, v1);
            if (m_GaeaRadialBlur.IsActive())
            {
                material.EnableKeyword(ShaderKeywordStrings.GAEA_RADIALBLUR);
            }
            else
            {
                material.DisableKeyword(ShaderKeywordStrings.GAEA_RADIALBLUR);
            }
        }
...
#region Internal utilities
        ...
        class MaterialLibrary
        {
            ...
            public readonly Material gaeaFXAA;
            ...
            public MaterialLibrary(PostProcessData data)
            {
                ...
                gaeaFXAA = Load(data.shaders.gaeaFxaaPS);
        ...
        static class ShaderConstants
        {
            ...
            public static readonly int _GAEARadialBlur_Params = Shader.PropertyToID("_GAEARadialBlur_Params");
