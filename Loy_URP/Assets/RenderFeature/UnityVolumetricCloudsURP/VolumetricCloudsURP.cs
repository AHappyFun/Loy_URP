...
public class VolumetricCloudsURP : ScriptableRendererFeature
{
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        // The effect is perspective-only. Rendering it again for an overlay camera would
        // also overwrite the base camera's temporal history and composite the clouds twice.
    ...
    public class VolumetricCloudsPass : ScriptableRenderPass
    {
        ...
        private sealed class CameraHistory
        {
            ...
            internal bool valid;
            internal int lastRenderedFrame = -1;
        }

        private readonly Dictionary<int, CameraHistory> cameraHistories = new();
    #endif
        ...
        private class PassData
        {
            ...
            internal CameraHistory cameraHistory;
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            if (data.denoiseClouds)
            {
                ...
                data.cloudsMaterial.SetTexture(volumetricCloudsHistoryTexture, data.historyHandle);
                data.cameraHistory.valid = true;
                data.cameraHistory.lastRenderedFrame = data.frameCount;
            }
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<PassData>(profilerTag, out var passData))
            {
                ...
                if (!cameraHistories.TryGetValue(cameraId, out CameraHistory cameraHistory))
                {
                ...
                bool historyReallocated = RenderingUtils.ReAllocateHandleIfNeeded(
                    ref cameraHistory.handle,
                    desc,
                    FilterMode.Point,
                    TextureWrapMode.Clamp,
                    name: $"{_VolumetricCloudsHistoryTexture}_{cameraId}");

                bool historyWasContinuous = cameraHistory.lastRenderedFrame == Time.frameCount - 1;
                bool resetRequested = camera.TryGetComponent(out UniversalAdditionalCameraData additionalCameraData) &&
                                      additionalCameraData.resetHistory;
                bool historyValid = cameraHistory.valid && !historyReallocated && historyWasContinuous && !resetRequested;
                TextureHandle historyHandle = renderGraph.ImportTexture(cameraHistory.handle);
                ...
                passData.cameraHistory = cameraHistory;
        ...
        #if UNITY_6000_0_OR_NEWER
            foreach (CameraHistory cameraHistory in cameraHistories.Values)
                cameraHistory.handle?.Release();
